...
if [ "$DIST" = "Ubuntu" ] || [ "$DIST" = "Debian" ]; then
    $install python3 python3-dev python3-pip python3-setuptools gcc make bird2
elif [ "$DIST" = "Fedora" -o "$DIST" = "RedHatEnterpriseServer" ]  || [ "$DIST" = "CentOS" ]; then
    $install python3 python3-devel python3-pip python3-setuptools gcc make bird
else
    echo "Unsupported distribution for automatic package installation."
    echo "Install Python 3, pip, Python development headers, gcc, make and bird manually."
fi
